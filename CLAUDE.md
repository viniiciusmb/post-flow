...
**Estilo de corte manual/interativo (migration 027, 2026-07-30):** cliente escolhe Automático (padrão de sempre) ou Manual em "Vídeos & Cortes → Estilo visual do corte". Manual libera: enquadramento contínuo arrastando (`crop_zoom_percent` 0-100, um único filtro ffmpeg cobre o que antes eram 2 modos binários `crop`/`blur_pad` — ver `buildFilter` em `videoEditingService.js`), galeria visual de estilo de legenda/título com 2 presets novos "balão" (`bubble_dark`/`bubble_purple`, fundo colorido atrás do texto via ASS `BorderStyle=3`), e numeração opcional "Parte N" em 6 posições (mapeadas direto pro `Alignment` do ASS). Testado visualmente via Playwright headless (Chrome do sistema + `playwright-core`, sem instalar nada globalmente) — capturei um bug real nesse processo: `PUT /api/client/video-settings` não devolvia `options` na resposta (só o `GET` devolvia), então qualquer save nessa tela ou na `VideoSettingsCard` antiga quebrava a página em seguida (`Cannot read properties of undefined`) — corrigido unificando num `toApiWithOptions()` usado pelos dois. Vale lembrar pra qualquer endpoint que devolve um objeto com sub-chave de opções fixas: GET e PUT/POST devem devolver o mesmo formato quando o front reusa a resposta pra atualizar o estado inteiro.
